...
from scrapers.mapi_scraper.http import _get_with_meta, _aget_with_meta, _ok, _err, _save_debug_item, logger, _ALLO_HEADERS
...
class AlloModule(BaseModule):
    ...
    async def async_scrape_url(
        ...
        logger.info(f"Fetching Allo URL: {url}", extra={"site": site})
        code, html, meta = await _aget_with_meta(site, url, extra_headers=_ALLO_HEADERS, parse_json=False, save_raw=debug, proxy=proxy)
